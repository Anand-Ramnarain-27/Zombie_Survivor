...
public class MetaProgressionUI : MonoBehaviour
{
        ...
        for (int i = 0; i < metaProgressionContainer.metaLevels.Count; i++)
        {
            if (!metaProgressionContainer.metaLevels[i].unlocked)
            {
                ...
                if (i != 0)
                    ...
                    currentAmount = GameManager.instance.metaProgressionManager.playerBaseStats.Find(x => x.metaProgressionSO == metaProgressionContainer.metaProgressionSO)?.baseValue ?? 0;
    ...
    private IEnumerator OnPointerDownRoutine()
    {
        ...
        LoadMetaUI(GameManager.instance.metaProgressionManager.metaProgressionContainers
            .Find(x => x.metaProgressionSO == metaProgressionContainer.metaProgressionSO));

        GameManager.instance.saveSystem.SaveMetaProgressionToInspector();
    }
}
